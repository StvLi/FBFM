...
def construct_lerobot_multi_processor(config, 
    ...
    ctx = mp.get_context("spawn")  # 关键：spawn 而不是默认的 fork
...
class LatentLeRobotDataset(LeRobotDataset):
    ...
    def _action_post_process(self, local_start_frame, local_end_frame, latent_frame_ids, action):
        ...
        elif self.config.env_type == 'real_robot':
            # 1.先右后左
            right_trans, right_rotvec, right_gripper = action[:, 0:3], action[:, 3:6], action[:, 6:7]
            left_trans, left_rotvec, left_gripper = action[:, 7:10], action[:, 10:13], action[:, 13:14]
            
            # 2. 轴角转四元数
            right_quat = R.from_rotvec(right_rotvec).as_quat()
            left_quat = R.from_rotvec(left_rotvec).as_quat()
            
            # 3. 拼成 7 维绝对位姿
            right_pose_7d = np.concatenate([right_trans, right_quat], axis=1)
            left_pose_7d = np.concatenate([left_trans, left_quat], axis=1)
            
            # 4. 计算相对起始帧的相对位姿
            right_action_rel = get_relative_pose(right_pose_7d).numpy()
            left_action_rel = get_relative_pose(left_pose_7d).numpy()
            
            # 5.拼成标准的 16 维: [左7, 左夹1, 右7, 右夹1]
            action = np.concatenate([left_action_rel, left_gripper, right_action_rel, right_gripper], axis=1)
            
            # 6. 配置 30 维映射 (因为上面标准化成了先左后右，所以映射依然非常整洁)
            self.config.inverse_used_action_channel_ids = [
                0, 1, 2, 3, 4, 5, 6,        # [0-6]   左臂 EEF (对应 left_action_rel)
                8, 9, 10, 11, 12, 13, 14,   # [7-13]  右臂 EEF (对应 right_action_rel)
                16, 16, 16, 16, 16, 16, 16, # [14-20] 左臂 Joints (填全0)
                16, 16, 16, 16, 16, 16, 16, # [21-27] 右臂 Joints (填全0)
                7,                          # [28]    左臂 Gripper (对应 left_gripper)
                15                          # [29]    右臂 Gripper (对应 right_gripper)
            ]
        else:
            raise ValueError(f"Unsupported env_type: {self.config.env_type}")
    ...
    def __getitem__(self, idx) -> dict:
        ...
        out_dict['actions'], out_dict['actions_mask'] = self._action_post_process(
            local_start_frame, local_end_frame, latent_frame_ids, ori_data_dict['actions'] 
        )
