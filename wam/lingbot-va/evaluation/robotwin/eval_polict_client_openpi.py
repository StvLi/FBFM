...
def main(usr_args):
    ...
    # Convert save_root to absolute path based on original working directory
    if not Path(save_root).is_absolute():
        save_root = str(original_cwd / save_root)
    ...
    model = WebsocketClientPolicy(host="localhost", port=usr_args['port'])
...
def eval_policy(task_name,
    ...
    while succ_seed < test_num:
        ...
        print('before expert_check...')
        if expert_check:
            try:
                print('before setup_demo')
                TASK_ENV.setup_demo(now_ep_num=now_id, seed=now_seed, is_test=True, **args)
                print('before play_once')
